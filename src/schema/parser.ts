...
const definitionRegex = new RegExp(
  `definition\\s*(?<name>${identifierRegex})\\s*{s*((?<inline>.*)})?`,
)
const relationRegex = new RegExp(
  `(?<relation>(relation\\s(?<name>${identifierRegex}):\\s+(?<type>(${relationTypeRegex}(\\s*[|]\\s*)?)+)\\s*?))+`,
  'g',
)
const typeRegex = new RegExp(
  `(((?<type>${identifierRegex})(#(?<relation>${identifierRegex}))?))+`,
  'g',
)
const permissionRegex = new RegExp(
  `permission\\s+(?<name>${identifierRegex})\\s*=\\s*(?<expression>(((\\s*${permissionOperatorsRegex}\\s*)?${identifierRegex}(${permissionNestedOpRegex})?))+)`,
  'g',
)
const permissionExpressionRegex = new RegExp(
  `(?<type>([a-zA-Z_0-9]+(?<nested>->[a-zA-Z_0-9]+)?)(\\s*(?<operator>[+&-])\\s*)?)`,
  'g',
)

function parseRelations(relation: string): RelationType[] {
  const relations: RelationType[] = []
  const relationMatches: RegExpGroups<'name' | 'type'>[] = [
    ...relation.matchAll(relationRegex),
  ]
  for (const relation of relationMatches) {
    ...
    const resources: ResourceType[] = type
      ...
      .split('|')
      .map((it) => {
        const matches: RegExpGroups<'relation' | 'type'>[] = [
          ...it.trim().matchAll(typeRegex),
        ]
        if (!matches.length) {
          throw Error('blah')
        }
...
function parsePermissions(permission: string): PermissionType[] {
  ...
  const permissionMatches: RegExpGroups<'name' | 'expression'>[] = [
    ...permission.matchAll(permissionRegex),
  ]
  for (const permission of permissionMatches) {
    ...
    const expressionMatches: RegExpGroups<'type' | 'nested' | 'operator'>[] = [
      ...expression.matchAll(permissionExpressionRegex),
    ]
    if (!expressionMatches.length) {
      throw Error(
        `Invalid expression found (permission=${name}): ${expression}`,
      )
    }
...
function parseSchema(schemaString: string): SchemaDefinition {
  ...
  while (i < lines.length) {
    ...
    const typeMatch: RegExpGroups<'inline' | 'name'> =
      line.match(definitionRegex)
    if (!typeMatch || !typeMatch.groups) {
      throw new Error(`Invalid definition syntax at line ${i + 1}`)
    }

    const {
      groups: { inline: body, name },
    } = typeMatch
